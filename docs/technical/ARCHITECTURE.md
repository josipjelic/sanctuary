...
> Last updated: 2026-07-13 (AI-guided journal reverted: ADR-007; OCEAN onboarding & morning messages: ADR-005; Supabase deploy GitHub Action; Lists subsystem planned: tasks #029–#036; Reminders subsystem: ADR-004; AI I/O observability: ADR-003)
> Version: 0.1.0
...
- **AI-Guided Journal** (ADR-006): shipped, then **reverted** on 2026-07-13 (ADR-007). Journal screens, edge functions, and the evening reminder were removed from the codebase; migration `007_journal.sql` remains in history and the journal tables remain in the database (inert, no code paths touch them).
