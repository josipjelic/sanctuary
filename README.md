...
**EAS Update previews (Slack / CI):** Pushes to `main` publish to the `preview` channel (see `.eas/workflows/preview-build.yml`). Slack posts a tap-to-open `exp://u.expo.dev/…/group/…` link (and an HTTPS link to the Updates page on expo.dev). **Not every group behaves the same in Expo Go:** if one group loads and another spins forever, compare **Runtime version** and included **platforms** for both groups on [expo.dev → Updates](https://expo.dev/accounts/context.hr/projects/sanctuary/updates), or check that commit for a startup JS error. Internal `preview` profile installs can open the installed app to pull the channel without using the group link.
