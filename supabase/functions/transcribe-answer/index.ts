/**
 * transcribe-answer edge function.
 * Transcribes audio to text only — no database writes, no topic assignment.
 * Used by the onboarding voice-input flow to fill question TextInputs.
 */
...
Deno.serve(async (req) => {
  ...
  logAiInfo({
    ...
    openrouter_request: sanitizeOpenRouterRequestForLog(requestBody),
  });
  ...
  try {
    ...
    if (!res.ok) {
      ...
      logAiError({
        ...
        response_summary: errText.slice(0, 200),
      });
    ...
    if (!transcript) {
      return jsonResponse({ error: "Empty transcription result" }, 502);
    }
