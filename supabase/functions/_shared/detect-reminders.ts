...
function buildReminderSystemPrompt(
  ...
  return `You are a reminder extraction assistant. Given a thought or note, extract any future time references that could become reminders. ${tzBlock}${timeBlock}For each reminder, extracted_text is a short DISPLAY TITLE (not a verbatim quote). Use 1–4 words when possible; capitalize like a title; omit dates, times, and filler ("I have to", "remember to", "tomorrow at 2 PM"). Focus on the core action or place—prefer the head noun or place when clear (e.g. "Bank", "Dentist"); otherwise a short verb phrase ("Call mum", "Submit report"). Match the thought's language. If there is only a vague time and no subject ("remind me tomorrow"), use "Reminder". Example: "I have to go to bank tomorrow 2 PM" → extracted_text "Bank", scheduled_at tomorrow 14:00 local.

Return ONLY valid JSON matching this schema: { "reminders": [ { "extracted_text": "string — short reminder title", "scheduled_at": "ISO 8601 datetime string" } ] }. If there are no future time references, return { "reminders": [] }. Interpret vague references ("next Monday", "Wednesday afternoon", "in 3 hours") in the user's local timezone${
