...
- [ ] Sign Up screen: email + password fields, validation, Supabase `signUp()` call, success state
- [ ] Sign In screen: email + password fields, validation, Supabase `signInWithPassword()` call
- [ ] Forgot Password screen: email field, Supabase `resetPasswordForEmail()` call, confirmation message
- [ ] Auth guard: unauthenticated users redirected to Sign In on app launch
- [ ] Authenticated users skip auth screens (session persisted via AsyncStorage)
- [ ] Log Out: accessible from settings/profile, calls `supabase.auth.signOut()`
- [ ] All screens use design system components and tokens from task #004
- [ ] Loading spinners shown during async auth calls
- [ ] Error messages displayed inline (not toast-only) — e.g., "Invalid credentials"
